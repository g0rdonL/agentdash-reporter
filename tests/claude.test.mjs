...
test('collectClaudeSessions - detects active session from process and file', async (t) => {
  ...
  try {
    ...
    assert.strictEqual(events[0].metadata.path, '/Users/gordon/dev/foo');
    assert.strictEqual(events[0].issue_title, 'foo');
    assert.ok(events[0].metadata.last_activity);

    // Privacy check
    assert.strictEqual(events[0].metadata.message, undefined);
    assert.strictEqual(JSON.stringify(events[0]).includes('hello'), false);
    assert.strictEqual(JSON.stringify(events[0]).includes('world'), false);
  } finally {
